...
<div class="card">
    <div class="card-header">
        <div class="card-title"><i class="bi bi-graph-up-arrow"></i> Laporan Penjualan (Selesai/Sudah Diambil)</div>
    </div>

    <form method="GET" action="{{ route('report.index') }}" style="margin-bottom:20px;">
        <div class="search-wrap" style="background:#f8fafc;padding:14px;border-radius:12px;border:1px solid var(--border);">
    ...
    <div class="grid grid-3" style="margin-bottom:24px;">
        <div class="card" style="margin-bottom:0; background:rgba(22,163,74,.1); border:1px solid rgba(22,163,74,.2);">
            <div style="font-size:14px;color:var(--text-muted);">Total Pendapatan (Periode Ini)</div>
            <div style="font-size:28px;font-weight:800;color:var(--primary);margin-top:4px;">
                Rp {{ number_format($totalRevenue, 0, ',', '.') }}
            </div>
        </div>
        <div class="card" style="margin-bottom:0; background:rgba(234,179,8,.1); border:1px solid rgba(234,179,8,.2);">
            <div style="font-size:14px;color:var(--text-muted);">Transaksi Selesai</div>
            <div style="font-size:28px;font-weight:800;color:var(--secondary);margin-top:4px;">
                {{ $orders->count() }} Order
            </div>
        </div>
    </div>
            ...
            <tbody>
                @forelse($orders as $i => $o)
                    <tr>
                        ...
                        <td><span style="font-weight:700;color:var(--primary-dark);font-family:monospace;">{{ $o->order_code }}</span></td>
                        ...
                        <td style="text-align:right;font-weight:600;color:var(--primary);">Rp {{ number_format($o->total, 0, ',', '.') }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" style="text-align:center;padding:40px;color:#64748b;">
                            <div style="font-size:28px;margin-bottom:10px;"><i class="bi bi-graph-down"></i></div>
